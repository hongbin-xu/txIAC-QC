...
if "data" in st.session_state:
    ...
    with st.container():
        ...
        with col2:
            ...
            st.session_state["data_v1"]["indicator"] = st.session_state["data_v1"]["ACP RUT AUTO COMMENT CODE" + st.session_state["suffixes"][0]]+"-"+st.session_state["data_v1"]["ACP RUT AUTO COMMENT CODE" + st.session_state["suffixes"][1]]
            df1 = st.session_state["data_v1"].groupby(by = "indicator").size().reset_index(name = "count").sort_values(by = "count", ascending = False)
            fig= px.bar(df1, x = "indicator", y = "count")
            st.plotly_chart(fig, use_container_width= True)
